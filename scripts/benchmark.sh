...
stats_FLMSERVER() {
  ...
  local prompt_processing_time=$(echo "$response" | jq '.usage.completion_tokens // 0')
  local prompt_per_second=$(echo "$response" | jq '.usage.prefill_duration_ttft // 0')
  local tokens_generation=$(echo "$response" | jq '.usage.decoding_duration // 0')
  local tokens_generation_time=$(echo "$response" | jq '.usage.prefill_speed_tps // 0')
  local tokens_generation_per_second=$(echo "$response" | jq '.usage.decoding_speed_tps // 0')


  echo "= $name ="
  echo "Prompt: $prompt"
    echo "Prompt generation:     $prompt_tokens tokens in ${prompt_processing_time}ms (${prompt_per_second} t/s)"
  echo "Generation: $tokens_generation tokens in ${tokens_generation_time}ms (${tokens_generation_per_second} t/s)"
  echo ""
...
stats_LLAMACPP() {
  ...
  local name="$1" response="$2" prompt="$3"
  local prompt_tokens=$(echo "$response" | jq '.timings.prompt_n // 0')
  ...
  local tokens_generation=$(echo "$response" | jq '.timings.predicted_n // 0')
  local tokens_generation_time=$(echo "$response" | jq '.timings.predicted_ms // 0')
  local tokens_generation_per_second=$(echo "$response" | jq '.timings.predicted_per_second // 0')
